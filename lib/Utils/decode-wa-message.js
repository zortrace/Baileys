...
/** NACK reason codes we send to the server (client â†’ server) */
...
/**
 * Server-side error codes returned in ack stanzas (server â†’ client) that we
...
export const SERVER_ERROR_CODES = {
    ...
    /** Stanza validation failure (SMAX_INVALID) â€” likely stale device session */
    SmaxInvalid: '479'
};
...
// Module-level map: outgoing @bot message ID â†’ messageSecret
...
export function decodeMessageNode(stanza, meId, meLid) {
    ...
    if (isPnUser(from) || isLidUser(from) || isHostedLidUser(from) || isHostedPnUser(from)) {
        if (recipient && !isJidMetaAI(recipient)) {
            ...
            // with `from` set to our own device but no `recipient` attribute â€”
