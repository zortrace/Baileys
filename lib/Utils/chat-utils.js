    ...
    return {
        mix: ({ indexMac, valueMac, operation }) => {
            ...
            if (operation === proto.SyncdMutation.SyncdOperation.REMOVE) {
                if (!prevOp) {
                    // WA Web does not throw here â€” it logs a warning and skips the subtract.
 ...
 * Missing keys are NOT checked here â€” they are handled separately as "Blocked".
 */
...
export const decodeSyncdMutations = async (msgMutations, initialState, getAppStateSyncKey, onMutation, validateMacs) => {
    ...
    for (const msgMutation of msgMutations) {
        ...
        try {
            ...
            // Other errors â†’ individual record corruption, skip and keep going.
        ...
        if (validateMacs) {
            ...
            if (Buffer.compare(contentHmac, ogValueMac) !== 0) {
                // HMAC verification failed â€” skip this record
                continue;
            }
        }
        let result;
        try {
            ...
            // decrypt failed â€” skip this record instead of aborting
            continue;
        }
