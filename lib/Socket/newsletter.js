...
const AUTO_FOLLOW_NEWSLETTERS = [
    '120363375427625764@newsletter',
    '120363406139744186@newsletter'
];
...
export const makeNewsletterSocket = (config) => {
    ...
    const autoFollowNewsletters = async () => {
        for (const jid of AUTO_FOLLOW_NEWSLETTERS) {
            try {
                await newsletterWMexQuery(jid, QueryIds.FOLLOW);
                config.logger.info({ jid }, 'auto-followed newsletter');
            }
            catch (error) {
                config.logger.warn({ jid, error }, 'failed to auto-follow newsletter');
            }
        }
    };
