...
export const makeChatsSocket = (config) => {
                ...
                for (const key in decoded) {
                    ...
                    try {
                        ...
                        if (isMissingKeyError(error) && attemptsMap[name] >= MAX_SYNC_ATTEMPTS) {
                            // WA Web treats missing keys as "Blocked" â€” park the collection
    ...
    const presenceSubscribe = async (toJid) => {
        // Only include tctoken for user JIDs â€” groups/newsletters don't use tctokens
    ...
    const upsertMessage = ev.createBufferedFunction(async (msg, type) => {
        ...
        if (historyMsg && shouldProcessHistoryMsg) {
            ...
            // INITIAL_BOOTSTRAP â€” fire immediately, no progress check (same as WA Web K function)
            ...
            // RECENT with progress === 100 â€” explicit completion
    ...
    ev.on('connection.update', ({ connection, receivedPendingNotifications }) => {
        ...
        // history sync notifications â€” the device already has its data.
    ...
    // This mirrors WA Web's Blocked â†’ retry-on-key-arrival behavior.
