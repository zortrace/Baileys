...
export const makeUsernameSocket = config => {
	 ...
	 * data.xwa2_username_check.result â†’ 'SUCCESS' | 'INVALID'
	...
	const checkUsername = async (username, includeSuggestions = true) => {
		if (!USERNAME_QUERY_IDS.CHECK) {
			throw new Error('Username CHECK query_id not configured â€” capture a live WA session to obtain it')
	}
	...
	const setUsername = async (username, options = {}) => {
		if (!USERNAME_QUERY_IDS.SET) {
			throw new Error('Username SET query_id not configured â€” capture a live WA session to obtain it')
	}
	 ...
	 * â†’ sending username=null triggers the delete path on the server.
	 */
	...
	const getMyUsername = async () => {
		if (!USERNAME_QUERY_IDS.GET) {
			throw new Error('Username GET query_id not configured â€” capture a live WA session to obtain it')
	}
	...
	const setUsernamePin = async pin => {
		if (!USERNAME_QUERY_IDS.PIN_SET) {
			throw new Error('Username PIN_SET query_id not configured â€” capture a live WA session to obtain it')
	}
