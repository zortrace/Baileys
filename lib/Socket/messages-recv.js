...
export const makeMessagesRecvSocket = (config) => {
     ...
     * Mirrors WAWebSendTcTokenWhenDeviceIdentityChange â€” runs in parallel with
    ...
    const handleBadAck = async ({ attrs }) => {
        ...
        if (attrs.error) {
            ...
            if (attrs.error === SERVER_ERROR_CODES.MessageAccountRestriction) {
                ...
                // No retry â€” retrying counts as another "reach out" and worsens the restriction.
                ...
                logger.warn({ msgId: attrs.id, from: attrs.from }, 'smax-invalid (479): stanza rejected by server â€” likely stale device session or malformed addressing');
            }
    ...
    /** timestamp of last tctoken prune run â€” throttles to once per 24h */
    ...
    ev.on('connection.update', ({ isOnline, connection }) => {
        ...
        if (connection === 'close' && tcTokenIndexTimer) {
            clearTimeout(tcTokenIndexTimer);
            tcTokenIndexTimer = undefined;
            // Best-effort flush â€” may fail if store is already closed
            try {
        ...
        // Note: don't gate on tcTokenKnownJids.size â€” the index may still be loading
        if (isOnline) {
            const now = Date.now();
    ...
    async function pruneExpiredTcTokens() {
        try {
            ...
            for (const jid of jids) {
                ...
                if (!entry) {
                    // Tracked but nothing in store â€” drop from index.
                    mutated++;
                    continue;
                }
