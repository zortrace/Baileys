    ...
    const relayMessage = async (
	    ...
	    await authState.keys.transaction(async () => {
		    ...
		    if (isGroupOrStatus &&!isRetryResend) {
	    		...
	    		// Fix: detect silent delivery failure â€” if the destination is genuinely
	    		...
	    		if (!isRetryResend && !isSelfChatDestination && otherRecipients.length === 0) {
	    			...
	    			throw new Boom('No devices resolved for recipient â€” message was not delivered to the other party', { statusCode: 421, data: { jid } })
	    		}
